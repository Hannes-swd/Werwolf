...
export default function LanguageSwitcher({ className = '' }: { className?: string }) {
  ...
  return (
    <div
      className={`ww-language-switcher relative inline-flex min-h-11 items-center rounded-xl border border-[var(--ww-border)] backdrop-blur-xl ${className}`}
    ...
    >
      ...
      >
        {SUPPORTED_LOCALES.map(option => (
          <option key={option} value={option} dir={LOCALE_DIRECTION[option]} className="bg-[var(--ww-surface)] text-[var(--ww-text)]">
